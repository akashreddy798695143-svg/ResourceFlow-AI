...
export function CommandCenterView() {
  ...
  const runDemo = async (
    scenario: 'EARTHQUAKE' | 'LANDSLIDE' | 'ROAD_BLOCKAGE' = 'EARTHQUAKE'
  ) => {
    setDemoRunning(true)

    const id = toast.loading(
      `Running full end-to-end ${scenario.toLowerCase()} demo…`
    )
    ...
    try {
      ...
      toast.success(`Demo complete · ${res.incidentCode}`, {
        id,
        description: `${res.steps.length} steps executed`,
      })
  ...
  const filteredIncidents = incidents.filter((i) => {
    ...
    if (riskFilter !== 'ALL' && i.riskLevel !== riskFilter) {
      return false
    }
  ...
  const activeRespondersCount = resources.filter((r) =>
    ['ASSIGNED', 'EN_ROUTE', 'ON_SCENE'].includes(r.status)
  ).length
  ...
  const stats = analytics || {
    ...
    assignedResources: resources.filter((r) =>
      ['ASSIGNED', 'EN_ROUTE', 'ON_SCENE'].includes(r.status)
    ).length,
    delayedResponses: incidents.filter(
      (i) => i.status === 'DELAYED'
    ).length,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2 p-3 md:px-6">
        ...
        <KPI
          label="Avg Response"
          value={stats.avgResponseTimeMin ? `${stats.avgResponseTimeMin}m` : '< 5m'}
          icon={Clock}
        />
      ...
      <div className="flex items-center gap-2 px-3 md:px-6 pb-2 flex-wrap">
        ...
        {[
          ...
          <button
            ...
            className={cn(
              'text-[10px] px-2 py-0.5 rounded-md border transition',
              typeFilter === t
                ? 'border-primary bg-primary/15 text-primary font-medium'
                : 'border-border text-muted-foreground hover:bg-accent/40'
            )}
          >
            {t === 'ALL' ? 'All Scenarios' : t.replace(/_/g, ' ')}
          </button>
        ))}
                    ...
                    {priorityQueue.map((inc) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {inc.riskLevel && (
                            <RiskBadge
                              level={inc.riskLevel as RiskLevel}
                              score={inc.riskScore ?? undefined}
                            />
                          )}
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          ...
                          {inc.duplicateFlag && (
                            <Badge
                              variant="outline"
                              className="text-[9px] text-sev-MEDIUM border-sev-MEDIUM"
                            >
                              DUP
                            </Badge>
                          )}
                          ...
                          {inc.escalationLevel > 0 && (
                            <Badge
                              variant="outline"
                              className="text-[9px] text-sev-CRITICAL border-sev-CRITICAL"
                            >
                ...
                {approvals.length === 0 ? (
                  ...
                  <div className="divide-y divide-border">
                    ...
                    {approvals.slice(0, 5).map((a) => {
                      ...
                      const rec = (() => {
                        try {
                          return JSON.parse(a.recommendation)
                        } catch {
                          return null
                        }
                      })()
                      ...
                      return (
                        <div key={a.id} className="p-3">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-mono text-xs text-primary">
                              {a.incident?.incidentCode}
                            </span>

                            <RiskBadge
                              level={a.incident?.riskLevel as RiskLevel}
                              score={a.incident?.riskScore}
                            />
                          ...
                          {recommended ? (
                            <div className="mt-1 text-xs">
                              ...
                              <span className="text-muted-foreground">
                                {' '}
                                · ETA {recommended.eta_minutes}min
                              </span>
                          ...
                          <div className="mt-2 flex items-center gap-2">
                            ...
                            <Badge
                              variant="outline"
                              className="text-[9px] text-sev-HIGH border-sev-HIGH"
                            >
                              APPROVAL REQUIRED
                            </Badge>
                ...
                <div className="flex gap-2 p-2 overflow-x-auto">
                  ...
                  {[...events]
                    ...
                    .map((e, i) => (
                      <div
                        key={i}
                        className="shrink-0 max-w-xs rounded-md border border-border bg-background/60 p-2 text-xs"
                      >
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <Badge
                            variant="outline"
                            className="text-[9px] font-mono text-primary border-primary/40"
                          >
...
function KPI({
  ...
  value: number | string
  icon: any
  tone?: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' | 'default'
}) {
  const toneClass =
    tone === 'CRITICAL'
      ? 'text-sev-CRITICAL'
